...
// узлы между подотрезками выделить другим цветом
// выводить в экспоненциальной форме 1.0*е(+/-).....
// все выводится в столбик - символ под символом
// нормализованный вывод, форматированный вывод

// абсолютная = точное минус приближенное, относительная = абсолютная погрешность делить на точное значение
// норма первая = сумма модулей, вторая = корень квадратный из суммы квадратов, через них считается погрешность - не по-компонентно
// считать между векторами погрешности, разность между координатами вектора = h/100, 6 чисел вывести - 3 нормы абс и отн погрешности
// погрешность в b можно не считать

// тесты - взять x^7 и посмотреть на 5, 9 точках - качество скакнет вверх
